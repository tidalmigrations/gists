...
require 'json'
module HttpUtil
  def make_json_request(method:, path:, query_params: {}, body: nil, headers: {}, ssl: true, timeout: 60, basic_auth: [])
  ...
  def basic_request(path:, query_params:{}, body: {}, headers: {})
...
module AzureMigrate
  ...
  def machines
    ...
    azure_api_request(method: :get, path: path, query_params: {"api-version": version}, responses: [])
  end
  ...
  def list_assessment_projects
    ...
    path = "https://management.azure.com/subscriptions/#{subscription}/resourceGroups/#{resource_group}/providers/Microsoft.Migrate/assessmentProjects?#{version}"
    assessments = basic_request(path:         path,
                                 body:         nil,
  ...
  def parse_result(result)
    ...
    if result
      ...
      properties["disks"].each do |key, disk_value|  
        gb_adder+= disk_value['gigabytesAllocated']
      end
      check_virtualization = properties["datacenterManagementServerName"] != nil
      ram_allocated_gb = (properties["megabytesOfMemory"]/1000).to_i
      ip_addresses = []

      properties["networkAdapters"].each do |k, v|
  ...
  private
    ...
    def azure_api_request(method:, path:, body: nil, query_params: {}, responses: [])
