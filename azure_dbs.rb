...
module HttpUtil
  ...
  module AzureDB
    ...
    def list_db_servers(subscription, resource_group)
      ...
      result = response_handler(api_name: "Azure Database Servers", response: response)["value"].map do |server|
        {
          name: server["name"],
    ...
    subscriptions.each do |subscription|
      ...
      resource_groups.each do |resource_group|
        ...
        db_servers.each do |server|
          STDERR.puts "Fetching databases for server #{server[:name]}..."
